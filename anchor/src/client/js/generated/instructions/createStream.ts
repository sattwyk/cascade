...
export const CREATE_STREAM_DISCRIMINATOR = new Uint8Array([
  71, 188, 111, 127, 108, 40, 229, 158,
]);

export function getCreateStreamDiscriminatorBytes() {
  return fixEncoderSize(getBytesEncoder(), 8).encode(
    CREATE_STREAM_DISCRIMINATOR
  );
}

export type CreateStreamInstruction<
  ...
  InstructionWithAccounts<
    [
      ...
      TAccountEmployee extends string
        ? ReadonlyAccount<TAccountEmployee>
        : TAccountEmployee,
...
export type CreateStreamInstructionData = {
  discriminator: ReadonlyUint8Array;
  hourlyRate: bigint;
  totalDeposit: bigint;
};

export type CreateStreamInstructionDataArgs = {
...
export function getCreateStreamInstructionDataEncoder(): FixedSizeEncoder<CreateStreamInstructionDataArgs> {
  return transformEncoder(
    ...
    (value) => ({ ...value, discriminator: CREATE_STREAM_DISCRIMINATOR })
  );
}
...
export function getCreateStreamInstructionDataCodec(): FixedSizeCodec<
  CreateStreamInstructionDataArgs,
  CreateStreamInstructionData
> {
  return combineCodec(
    getCreateStreamInstructionDataEncoder(),
    getCreateStreamInstructionDataDecoder()
  );
}
...
export async function getCreateStreamInstructionAsync<
  TAccountEmployer extends string,
  TAccountEmployee extends string,
  TAccountMint extends string,
  TAccountStream extends string,
...
> {
  ...
  const originalAccounts = {
    ...
    employee: { value: input.employee ?? null, isWritable: false },
  ...
  if (!accounts.stream.value) {
    accounts.stream.value = await getProgramDerivedAddress({
      programAddress,
      seeds: [
        getBytesEncoder().encode(new Uint8Array([115, 116, 114, 101, 97, 109])),
        getAddressEncoder().encode(expectAddress(accounts.employer.value)),
        getAddressEncoder().encode(expectAddress(accounts.employee.value)),
      ],
    });
  }
  ...
  return Object.freeze({
    ...
    data: getCreateStreamInstructionDataEncoder().encode(
      args as CreateStreamInstructionDataArgs
    ),
    programAddress,
...
export function getCreateStreamInstruction<
  TAccountEmployer extends string,
  TAccountEmployee extends string,
  TAccountMint extends string,
  TAccountStream extends string,
...
export type ParsedCreateStreamInstruction<
  TProgram extends string = typeof CASCADE_PROGRAM_ADDRESS,
...
export function parseCreateStreamInstruction<
...
>(
  instruction: Instruction<TProgram> &
    ...
    InstructionWithData<ReadonlyUint8Array>
): ParsedCreateStreamInstruction<TProgram, TAccountMetas> {
  ...
  return {
    ...
    data: getCreateStreamInstructionDataDecoder().decode(instruction.data),
  };
}
