...
import { employees, employeeStatusHistory, organizationActivity, streamEvents, streams } from '@/core/db/schema';
...
export async function getAuditTrail(
  ...
  try {
    const statusHistoryPromise = drizzleClientHttp
      .select({
    ...
    const streamEventsPromise = drizzleClientHttp
      .select({
        id: streamEvents.id,
    ...
    const orgActivityPromise = drizzleClientHttp
      .select({
    ...
    const [statusHistoryResult, streamEventsResult, orgActivityResult] = await Promise.allSettled([
