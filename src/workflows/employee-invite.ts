...
import { employeeInvitations, employees, organizationUsers, type employmentTypeEnum } from '@/db/schema';
...
async function dispatchInviteEmail(input: DispatchInviteEmailInput) {
  ...
  const { EmployeeInviteEmailTemplate } = await import('@/email/employee-invite-email');
